...
ask(E,_):-stop(E),fail.
\end{code}
Note  the checking for terminated generators by {\tt is\_done/1} and the explicit termination by {\tt stop/1} on failure.
...
\end{codex}
that, when {\tt ask/2} is called, it produces the next element with
\begin{codex}
call(=(Constant),X)
\end{codex}
Thus, call acts in this case by unifying {\tt X} and {\tt Constant}.

Another constructor, relying this time on an externally maintained state is {\tt rand/1}.
\begin{code}
rand(random()).
\end{code}
This generator, when called by {\tt ask/2}, will produce a stream of random floating point numbers between 0 and 1.
...
gen_next(F,State,X):-
  ...
  nb_setarg(1,State,Y).
\end{code}
where {\tt State} acts as a container for destructively updated values.
...
Implementing the engine API does not need  a Prolog system supports multi-threading. It only assumes that the virtual machine is fully reentrant, it can be queried and that it can stop, yield data and resume execution as a coroutine.
...
acts as a neutral element for product.
...
evaluates a generator expression to ready to use generator that combines 
...
With most stream generators no garbage collection is needed when working destructively in constant space, but results can be exposed declaratively via the stream algebra.
...
%Generators can bring in languages like Python \\-e.g. itertools, 

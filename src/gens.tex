...
order of the elements is unimportant, it is also commutative.  Also, is thas
...
is is paired with the previously produced elements of the other generator,
which a stored in a list.
\begin{code}
...
and if the order of the elements does not matter.  Finally, under the same
...
\begin{tabular}{rcl@{\hspace{0.4cm}}l@{\hspace{1cm}}cl@{\hspace{0.4cm}}l}
  $F$    & ::=  & $F_1 + F_2$         & \it (sum)         & $\mid$ & $\{F\}$           & \it (setification)\\
         ...
         & $\mid$ & $[X|Xs]$          & \it (list)        & $\mid$ & $E$               & \it (stream)
\end{tabular}
\end{center}
...
\end{code}
This works for infinite generators (upto the limits of available memory), in contrast
...
\end{code}
Here is a small example that combines many of the features:
\begin{codex}
?- X in_ ({[a,b,a]}+(1:3)*c).
...
This section describes the additional strream operations that our {\tt
...
creates a generator that reduces a given generator's elements with the given closure, 
